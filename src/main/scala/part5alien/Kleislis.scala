...
object Kleislis {
  ...
  val plainFunc1: Int => String = x => if (x % 2 == 0) s"$x is even" else ""
  val plainFunc2: Int => Int    = x => x * 3
  val plainFunc3: Int => String = plainFunc2 andThen plainFunc1

  import cats.data.Kleisli
  import cats.instances.option.* // implicit FlatMap[Option]
  ...
  // hint
  val times2   = Reader[Id, Int, Int](x => x * 2)
  val plus4    = Reader[Id, Int, Int](y => y + 4)
